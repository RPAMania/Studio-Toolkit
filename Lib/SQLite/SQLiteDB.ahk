...
Class SQLiteDB {
   ...
   Class _Statement {
      ...
      Step() {
         ...
         If (RC <> This._DB.ReturnCode("SQLITE_DONE"))
         && (RC <> This._DB.ReturnCode("SQLITE_ROW")) {
            This.ErrorMsg := This._DB.ErrMsg()
            This.ErrorCode := RC
            Return False
         }
   ...
   ; ===================================================================================================================
   __New(iniFilePath := "") {
      static WINERROR := { ERROR_BAD_EXE_FORMAT: 0xC1, ERROR_MOD_NOT_FOUND: 0x7E }
      ...
      If (This.Base._RefCount = 0) {
         ...
         If !FileExist(SQLiteDLL) {
            If FileExist(A_ScriptDir . "\SQLiteDB.ini") {
               iniFilePath := A_ScriptDir . "\SQLiteDB.ini"
            } else if (iniFilePath != "" && FileExist(iniFilePath)) {
            } else {
               MsgBox, 16, SQLiteDB Error, % "DLL " . SQLiteDLL . " not found!"
               ExitApp
            }

            IniRead, SQLiteDLL, % iniFilePath, Main, DllPath, %SQLiteDLL%
            SQLiteDll := Format(SQLiteDll, a_ptrsize == 8 ? "x64" : "x86")
         }
         If !(DLL := DllCall("LoadLibrary", "Str", SQLiteDLL, "UPtr")) {
            ...
            switch (a_lasterror)
            {
               case WINERROR.ERROR_BAD_EXE_FORMAT:
                  throw Exception("DLL " SQLiteDLL " is not a " 
                     . (a_ptrsize == 8 ? "64-" : "32-") "bit binary!", 
                        , This.ReturnCode("SQLITE_BADDLLFORMAT"))
               case WINERROR.ERROR_MOD_NOT_FOUND:
                  throw Exception("DLL " SQLiteDLL " does not exist!", 
                     , This.ReturnCode("SQLITE_NODLL"))
               default:
                  throw Exception("Unidentified issue during loading the DLL file " SQLiteDll)
            }
         }
         ...
         If (SQLVersion[1] < MinVersion[1]) || ((SQLVersion[1] = MinVersion[1]) && (SQLVersion[2] < MinVersion[2])){
            ...
            throw Exception("Version " . This.Base.Version . " of SQLite3 DLL is not supported!`n`n"
               . "You can download the current version from www.sqlite.org!")
         }
      }
   ...
   __Delete() {
      If (This._Handle)
         This.CloseDB()
      This.Base._RefCount := max(This.Base._RefCount - 1, 0)
      If (This.Base._RefCount = 0) {
   ...
   ; ===================================================================================================================
   ReturnCode(RC) {
      Static RCODE := {SQLITE_BADDLLFORMAT: -2 ; Unsuitable DLL format
                     , SQLITE_NODLL: -1      ; DLL file missing
   ...
   GetTable(SQL, ByRef TB, MaxResult := 0) {
      ...
      If (MaxResult < -2)
         MaxResult := 0
      This._StrToUTF8(SQL, UTF8)
      RC := DllCall(This.Base._SQLiteDLL "\sqlite3_get_table", "Ptr", This._Handle, "Ptr", &UTF8, "UPtrP", Table
   ...
   Query(SQL, ByRef RS) {
      ...
      If (RC < 1) {
         ...
         This.ErrorCode := This.ReturnCode("SQLITE_EMPTY")
         Return False
      }
      ColumnCount := RC
      Names := []
      ...
      If (RC = This.ReturnCode("SQLITE_ROW"))
         HasRows := True
   ...
   StoreBLOB(SQL, BlobArray) {
      ...
      If (RC) && (RC <> This.ReturnCode("SQLITE_DONE")) {
         This.ErrorMsg := A_ThisFunc . ": " . This._ErrMsg()
         This.ErrorCode := RC
         Return False
      }
...
SQLiteDB_RegExp(Context, ArgC, Values) {
   ...
   DllCall(This.Base._SQLiteDLL "\sqlite3_result_int", "Ptr", Context, "Int", !!Result, "Cdecl") ; 0 = false, 1 = true
}